...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None):
    ...
    elif data_source in ["length_control"]:
        from. import length_control
        # res = length_control.compute_score(solution_str, ground_truth,extra_info["split"])# 调用critic计算分数函数    
        # [+] 从 extra_info 中解包，并设置默认值以确保安全
        current_step = extra_info.get("current_step", -1)
        total_steps = extra_info.get("total_steps", -1)
        linear_truth = extra_info.get("linear_truth", True)

        # 将 step 信息作为独立的参数，传递给最终的函数
        res = length_control.compute_score(
            solution_str, 
            ground_truth, 
            extra_info["split"], 
            current_step=current_step,
            total_steps=total_steps,
            linear_truth=linear_truth,
        )
        # [+] 调用critic计算分数函数,并传入参数
